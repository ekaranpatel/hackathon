...
export default function StudentResourcePage() {
  ...
  // Fetch current user's existing bookings on mount (Safe Cleanup)
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchUserBookings = async () => {
      try {
        ...
        if (isMounted && (res.data?.success || Array.isArray(res.data))) {
          setUserBookings(res.data.bookings || res.data || []);
        }
      } catch (err) {
        if (isMounted) console.warn('Could not load user bookings:', err.message);
      }
    };
  ...
  // 🔒 UPDATED BOOKING SUBMISSION HANDLER WITH SAFE DATE NORMALIZATION
  const handleBookingSubmit = useCallback(async (resource, slotInput, dateInput) => {
    const resourceId = resource._id || resource.id;

    // Normalize slot input into an array of labels/strings
    const rawSlots = Array.isArray(slotInput) ? slotInput : [slotInput];
    const selectedSlotsList = rawSlots.map((s) =>
      typeof s === 'object' ? s.id || s.time || s.label : s
    );

    const bookingDate = dateInput || new Date().toISOString().split('T')[0];

    // 1. Client-Side Check: Verify if user already booked this exact resource & any selected slot
    const conflictingSlot = selectedSlotsList.find((slotLabel) =>
      userBookings.some((b) => {
        ...
        const bDate = normalizeDate(b.bookingDate || b.dateISO || b.date);
        const bStatus = String(b.status || '').toLowerCase();

        return (
          String(bResId) === String(resourceId) &&
          String(bSlot).trim() === String(slotLabel).trim() &&
          (!bDate || bDate === bookingDate) &&
          bStatus !== 'cancelled' &&
          bStatus !== 'rejected'
        );
      })
    ...
    if (conflictingSlot) {
      alert(`You have already booked "${resource.name}" for slot (${conflictingSlot}). Multiple bookings for the same slot are not allowed.`);
      return;
    }

    // 2. Submit Booking Request to Server
    try {
      ...
      if (response.data.success || response.status === 201) {
        alert(`Success! Booking confirmed for "${resource.name}" for ${selectedSlotsList.length} slot(s).`);

        const createdBookings = Array.isArray(response.data.bookings)
          ? response.data.bookings
          : selectedSlotsList.map((slotLabel) => ({
              resource: resourceId,
              slot: slotLabel,
              date: bookingDate,
              status: 'pending',
        ...
        setUserBookings((prev) => [...prev, ...createdBookings]);
      ...
      const errorMessage =
        ...
        'Selected slot(s) are unavailable.';

      alert(`Booking Failed: ${errorMessage}`);
    } finally {
      setBookingLoading(false);
    }
  }, [userBookings, refetchResources]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 min-h-screen text-slate-100">
      {/* Top Banner */}
            ...
            <div className="flex items-center gap-1.5 bg-slate-950 border border-slate-800 px-2.5 py-1.5 rounded-lg">
              ...
              >
                ...
                {(labsList || []).map((lab, index) => {
                  const labValue = lab._id || lab.id || lab.name || index;
      ...
      {filteredResources.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredResources.map((resource, idx) => {
            ...
            const isGeneral = String(resource.category || '').trim().toLowerCase() === 'general';

            const resolvedLocation = isGeneral
              ? resource.location || resource.loc || 'General Area'
              : typeof resource.lab === 'object'
              ? resource.lab?.name || resource.lab?.labName
              : resource.labName || resource.lab || 'Assigned Lab';

            const resourceKey = resource._id || resource.id || `${resource.name}-${idx}`;
